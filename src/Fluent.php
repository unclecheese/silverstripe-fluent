 ...
 */
class Fluent implements TemplateGlobalProvider
{
    use Injectable;
    use Configurable;
    use Extensible;
    ...
    public static function install_locale($persist = true)
    {
        ...
        foreach (array(LC_COLLATE, LC_CTYPE, LC_MONETARY, LC_TIME) as $category) {
            $l = preg_match('/^tr_/', $locale) ? 'en_US' : $locale;
            setlocale($category, "{$l}.UTF-8", $l);
        }

        i18n::config()->date_format = 'MM-dd-YYYY';
    }
